...
class Attention(nn.Module):
        ...
        if ca_attention == 1:
            ...
            for i in range(self.ca_num_heads):
                local_conv = nn.Conv2d(dim // self.ca_num_heads, dim // self.ca_num_heads, kernel_size=(3 + i * 2),
                                       padding=(1 + i), stride=1, groups=dim // self.ca_num_heads)  # kernel_size 3,5,7,9 大核dw卷积，padding 1,2,3,4
    ...
    def forward(self, x, H, W):
        ...
        if self.ca_attention == 1:
            v = self.v(x)
            s = self.s(x).reshape(B, H, W, self.ca_num_heads, C // self.ca_num_heads).permute(3, 0, 4, 1, 2)  # num_heads,B,C,H,W
            for i in range(self.ca_num_heads):
...
if __name__ == '__main__':
    ...
    model = smt_t()
    # solution 2
    model_path = '../ckps/smt/smt_tiny.pth'
    device = 'cuda:0'
    model.load_state_dict(torch.load(model_path)['model'])
    torch.save(model.state_dict(), "../ckps/smt.pth")
    # model2 = torch.load(model_path, map_location=device)["model"]
    # new_pth = dict()
    # for i in model2:
    #     for k,v in i:
    #         new_pth[k] = v
